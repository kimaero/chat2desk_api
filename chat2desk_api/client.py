...
class Client:
    ...
    def _communicate(self, verb, url, **kwargs):
        ...
        try:
            ...
            log.exception(f'API response is not JSON:\n'
                          ...
                          f' - request: {pformat(kwargs)}\n'
                          f' - content: {response.text}')

    def get__messages(self, message_id: int = None, transport: Transport = None, channel_id: int = None,
    ...
    def post__web_hook(self, webhook_url: str):
        return self.post(
            url=URL('v1/companies/web_hook'),
            data=non_empty_dict(url=webhook_url)
        )

    def post__chat_close_web_hook(self, webhook_url: str):
        return self.post(
            url=URL('v1/companies/chat_close_web_hook'),
            data=non_empty_dict(url=webhook_url)
        )

    def post__messages(self, client_id: int = None, text: str = None, transport: Transport = None,
    ...
    def post__messages_inbox(self, channel_id: int = None, body: str = None, image: str = None, video: str = None,
    ...
    def post__messages_assign(self, message_id: int, operator_id: int):
        return self.post(
            url=URL(f'v1/messages/{message_id}/assign'),
            data=non_empty_dict(operator_id=operator_id)
        )

    def get__clients(self, limit: int = None, offset: int = None):
        return self.get(
            url=URL(f'v1/clients'),
            params=non_empty_dict(limit=limit, offset=offset)
        )

    def get__client(self, client_id: int, extra: str = '', **kwargs):
        return self.get(
            url=URL(f'v1/clients/{client_id}' + (f'/{extra}' if extra else '')),
            **kwargs
        )

    def get__client_transport(self, client_id: int):
        return self.get__client(client_id=client_id, extra='transport')

    def get__client_last_question(self, client_id: int):
        return self.get__client(client_id=client_id, extra='last_question')

    def get__client_dialogs(self, client_id: int, limit: int = None, offset: int = None):
        return self.get__client(client_id=client_id, extra='dialogs',
                                params=non_empty_dict(limit=limit, offset=offset))

    def get__client_by_phone(self, phone: str):
        return self.get(
            url=URL(f'v1/clients'),
            params=non_empty_dict(phone=phone)
        )

    def get__clients_by_tags(self, tags: List[str], limit: int = None, offset: int = None):
        return self.get(
            url=URL(f'v1/clients'),
    ...
    def post__clients(self, phone: str, transport: Transport, channel_id: int):
        return self.post(
            url=URL('v1/clients'),
            params=non_empty_dict(phone=phone, transport=transport, channel_id=channel_id)
        )

    def put__client(self, client_id: int, nickname: str = None, comment: str = None, extra_comment_1: str = None,
    ...
    def get__operators(self, phone: str = None, email: str = None, online: bool = None, limit: int = None,
                       offset: int = None):
        return self.get(
    ...
    def get__dialogs(self, dialog_id: int = None, operator_id: int = None, state: DialogState = None,
    ...
    def get__unanswered_dialogs(self, seconds_to_overdue: int = 600):
        return self.get(
            url=URL('v1/dialogs/unanswered'),
            params=non_empty_dict(limit=seconds_to_overdue)
        )

    def put__dialog(self, dialog_id: int, operator_id: int = None, state: DialogState = None):
        return self.put(
            url=URL(f'v1/dialogs/{dialog_id}'),
            data=non_empty_dict(operator_id=operator_id, state=state)
        )

    def get__tags(self, tag_id: int = None, limit: int = None, offset: int = None):
        return self.get(
            url=URL('v1/tags' + (f'/{tag_id}' if tag_id else '')),
            params=non_empty_dict(limit=limit, offset=offset)
        )

    def post__assign_tags_to_client(self, tag_ids: List[int], client_id: int):
        return self.post(
            url=URL('v1/tags/assign_to'),
            data=non_empty_dict(tag_ids=tag_ids, assignee_type='client', assignee_id=client_id)
        )

    def delete__tags_from_client(self, tag_id: int, client_id: int):
        return self.post(
            url=URL(f'v1/tags/{tag_id}/delete_from'),
            data=non_empty_dict(client_id=client_id)
        )

    def get__templates(self, template_id: int = None, limit: int = None, offset: int = None):
        return self.get(
            url=URL('v1/templates' + (f'/{template_id}' if template_id else '')),
            params=non_empty_dict(limit=limit, offset=offset)
        )

    def post__qr_decode(self, image_url: str):
        return self.post(
